...
if (!class_exists("ReadDrives")) {
    ...
    class ReadDrives {
        ...
        function __construct($url, $htmlCode, $nombreCarpeta) {
            $this->url = $url;
        ...
        function Google() {
            ...
            if ($e) {
                ...
                $currentFolder['name'] = rtrim($div_contenedor->find('.flip-folder-title', 0)->plaintext);
                $currentFolder['n_files'] = $div_contenedor->find('.flip-folder-item-count', 0)->plaintext;

                $div_contenedor->find('.flip-entries', 0)->class = "flip-entries CFS_Content";
                ...
                foreach ($div_contenedor->find('
                    ...
                    .flip-entry-thumb') as $txt_version) {
                    $txt_version->outertext = '';
                }
                $div_contenedor->find('script', -1)->outertext = "";
                //$div_contenedor->find('script',-1)->class="";
            }
            $header = $htmlCode->find('.flip-header', 0);
            if ($header) {
                $div_header = str_get_html($header->outertext);
                $div_header->find('.flip-header-button', 0)->outertext = "";
                $div_header->find('a', 0)->href = $this->url;
                $div_header->find('a', 0)->target = "new";
                if($this->nombreCarpeta){
                    $n_Carpeta = $this->nombreCarpeta;
                }else{
                    $n_Carpeta = $currentFolder['name'];
                }
                $txt = $div_header->find('.flip-header-logo a', 0)->innertext . "<span class='drive-folder'>://" . $n_Carpeta . "/</span><div class='cloud-link'></div>";
                $div_header->find('.flip-header-logo a', 0)->innertext = $txt;
            }
            return $div_header . $div_contenedor;
        }
